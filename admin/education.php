...
if (empty($_GET['action']) === true) {
	echo '<h2>Edukacja</h2><hr class="style-one"></hr>';
	$list = $db->select_multi('SELECT * FROM ITEMS WHERE TYPE = "WORK"');
	...
	if (empty($list) === true) {
		...
		foreach ($list as $u) {
			...
			<td><a href="?page=education&action=edit&id='.$u['ID'].'"><input type="submit" class="btn btn-xs btn-primary" value="Edytuj"></a> <a href="?page=education&action=delete&id='.$u['ID'].'"><input type="submit" class="btn btn-xs btn-danger" value="Usuń"></a></td>
			</tr>';
		}
	}
	...
	echo '<a href="?page=education&action=add"><input type="submit" class="btn btn-primary" value="Dodaj"></a>';
...
<form action="" method="POST">
	<div class="row">
		<div class="col-lg-12">
			<h3>Edytuj edukację</h3><hr class="style-one"></hr>
			...
			<a href="?page=education"><button type="button" class="btn btn-warning btn-ls">Powrót</button></a>
		</div>
	</div>
</form>
<?php
	...
	if (empty($_POST) === false) {
		if (empty($_POST['name']) === false) {
			$db->query("INSERT INTO `ITEMS` (`TITLE`, `YEARS`, `TEXT`, `TYPE`) VALUES ('".$_POST['name']."', '".$_POST['years']."', '".$_POST['text']."', 'WORK');");
...
<form action="" method="POST">
	<div class="row">
		<div class="col-lg-12">
			<h3>Dodaj edukację</h3><hr class="style-one"></hr>
			<div class="form-group">
				<label>Nazwa</label>
